...
export default function calculateTripleQuotedStringPadding(source: string, stream: BufferedStream): Array<SourceLocation> {
  ...
  let sharedIndent = getIndentInfoForFragments(paddingTracker.fragments);

  if (firstContent.includes('\n') && isWhitespace(firstContent.split('\n')[0])) {
...
function getIndentInfoForFragments(fragments: Array<TrackedFragment>): string {
 ...
 * exactly two lines, we remove the code to remove the first line but not the
 * last line.
 */
